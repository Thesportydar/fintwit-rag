...
data "aws_ssm_parameter" "qdrant_snapshot_id" {
...
resource "aws_ebs_volume" "qdrant" {
  ...
  size              = 30
  type              = "gp3"
  snapshot_id       = data.aws_ssm_parameter.qdrant_snapshot_id.value != "" ? data.aws_ssm_parameter.qdrant_snapshot_id.value : null
