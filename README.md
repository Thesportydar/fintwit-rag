...
- `/lambdas/`: Serverless microservices (`agent/`, `pipeline/`, and layer builder `build-layers.sh`).
- `/tests/`: Automated test suite (fast unit tests with mocks and live decoupled E2E).
- `/evals/`: DeepEval LLM-as-a-Judge benchmark suite.
- `/scripts/`: Operational scripts (historical backfill and sparse vector migrations).
